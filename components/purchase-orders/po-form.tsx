...
export function POForm({ suppliers, onSubmit, onCancel, loading }: POFormProps) {
  const products = MOCK_PRODUCTS_WITH_INVENTORY.data
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
        <div className="grid grid-cols-2 gap-4">
          ...
          <FormField control={form.control} name="expected_date" render={({ field }) => (
            <FormItem>
        ...
        <div>
                ...
                <div className="col-span-3">
                  <FormField control={form.control} name={`items.${index}.cost`} render={({ field: f }) => (
                    <FormItem>
                      ...
                      <FormControl><Input type="number" min={0} step={1000} className="h-8 text-xs" {...f} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                </div>
          ...
          {totalCost > 0 && (
            <div className="mt-3 pt-3 border-t flex justify-end">
              <p className="text-sm">
                Total Cost: <span className="font-bold text-foreground ml-1">{formatCurrency(totalCost)}</span>
              </p>
            </div>
          )}
        </div>
